...
-- Allow authenticated users to insert workflows into their projects (Owner or invited member with edit access)
      ...
      EXISTS (
        ...
        AND (
          ...
          OR EXISTS (
            ...
            WHERE pm.project_id = p.id
            AND pm.user_id = auth.uid()
            AND (pm.access = 'edit' OR pm.role IN ('freelancer', 'owner'))
          )
        )
      )
    )
...
-- Allow authenticated users to delete workflows from their projects (Owner or invited member with edit access)
